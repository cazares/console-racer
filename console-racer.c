...
{	
	...
	for( i = 0; i < screen.screenHeight; i++ )
	{
		
		screen.track[i] = "";
		for(  j = 0; j < screen.leftWall; j ++ )
        {
            screen.track[i] = strcat(screen.track[i]," ");
        }
		screen.track[i] = strcat(screen.track[i],"|") ;
		for( j = screen.leftWall+1; j < screen.rightWall; j ++ )
        {
            screen.track[i] = strcat(screen.track[i]," ");
        }
		screen.track[i] = strcat(screen.track[i],"|");
		for( j = screen.rightWall+1; j < screen.screenWidth; j++ )
        {
            screen.track[i] = strcat(screen.track[i]," ");
        }
	}
...
void drawCar( screenData *screen, bool crashOccurred )
{
   ...
   char* currentTrack = screen->track[carY];
...
void updateTrack( screenData *screen )
{
   ...
   int carX = screen->currCarPositionX;
   int carY = screen->currCarPositionY;
   char* prevCarTrack = screen->track[prevCarY];
   char* carTrack = screen->track[carY];
   ...
   screen->track[prevCarY] = prevCarTrack;
    ...
    for( i = screen->screenHeight-1; i > 0; i--)
    {
    ...
    for( j = 0; j < screen->leftWall; j++)
    {
        screen->track[0] = strcat(screen->track[0]," ");
    }
    ...
    screen->track[0] = strcat(screen->track[0], nextLeftWall);
    ...
    screen->track[0] = strcat(screen->track[0], nextRightWall);
    for( j = screen->rightWall+1; j < screen->screenWidth; j++)
    {
        screen->track[0] = strcat(screen->track[0]," ");
    }
